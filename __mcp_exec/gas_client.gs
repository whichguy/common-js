    ...
    return new Proxy(target, {
      get(target, prop) {
        ...
        return function(...args) {
          ...
          if (debug) {
            log(`[GasServer][${requestId}] Creating call to ${prop}() with ${args.length} arguments`);
          }
          ...
          const api = {
            ...
            then(onResolve, onReject) {
              ...
              if (executionPromise) {
                if (debug) {
                  log(`[GasServer][${requestId}] Reusing cached promise for ${prop}()`);
                }
              ...
              if (!hasErrorHandler && throwOnUnhandled !== false) {
                if (debug) {
                  log(`[GasServer][${requestId}] No error handler provided, will auto-throw on rejection`);
                }
                finalOnReject = (error) => {
                  throw error;
                };
              ...
              if (debug) {
                log(`[GasServer][${requestId}] Executing ${prop}() with args:`, args);
              }
              ...
              try {
                ...
                executionPromise = new Promise((resolve, reject) => {
                  ...
                  const successHandler = (result) => {
                    ...
                    if (debug) {
                      log(`[GasServer][${requestId}] ${prop}() succeeded in ${duration}ms:`, result);
                    }
                  ...
                  // FIX #3: Only call withUserObject if defined
                  const runner = target
                    .withSuccessHandler(successHandler)
                    .withFailureHandler(failureHandler);

                  if (userObject !== undefined) {
                    runner.withUserObject(userObject);
                  }
