  ...
  <script>
      ...
      run: function(serverCode, clientCommand) {
        ...
        return new Promise((resolve, reject) => {
          if (!google || !google.script || !google.script.run) {
            ...
            window.UI.addOutput('Cannot execute: server not available', 'error', null, 'client: ' + clientTime + 'ms');
            reject(new Error('google.script.run not available'));
            return;
          }

          window.server.invoke(serverCode)
            .then(function(response) {
              const clientTime = (performance.now() - startTime).toFixed(2);
              
              // Validate response exists before accessing properties
              if (!response) {
                window.UI.addOutput('Error: No response from server', 'error', null, 'client: ' + clientTime + 'ms');
              ...
              if (response.success !== true) {
                const errorMsg = response.error || response.message || String(response.result) || JSON.stringify(response);
                window.UI.addOutput(
                  'Error: ' + errorMsg,
                  'error',
    ...
    window.DataLoader = {
      loadScriptInfo: function() {
        ...
        window.server.exec_api(null, 'common-js/__mcp_exec', 'getScriptInfo')
          .then(function(info) {
            if (!info || !info.scriptId) {
              console.error('Invalid script info response:', info);
              $('#scriptId').text('Invalid response');
              $('#scriptName').text('Invalid response');
              return;
            }
          ...
          .catch(function(error) {
            ...
            $('#scriptId').text('Error: ' + (error.message || 'Unknown error'));
      ...
      loadProcesses: function() {
        ...
        window.server.exec_api(null, 'common-js/__mcp_exec', 'getRecentProcesses')
          .then(function(response) {
            window.DataLoader.loadingProcesses = false;
            if (!response || !response.success) {
              $processes.html('<div style="padding: 8px; color: var(--error-red); font-size: 11px;">' + 
                (response && response.error ? response.error : 'Failed to load processes') + 
                '</div>');
              return;
            }

            // Validate processes is an array
            if (!Array.isArray(response.processes)) {
              console.error('Invalid processes response:', response);
              $processes.html('<div style="padding: 8px; color: var(--error-red); font-size: 11px;">Invalid response format</div>');
              return;
            }
      ...
      loadLogs: function(minutes) {
        ...
        if (isNaN(validMinutes) || validMinutes < 0) {
          console.error('Invalid minutes parameter:', minutes);
          $content.html('<div class="log-empty">Invalid time range parameter</div>');
          $status.text('Error');
          return;
        }
        ...
        window.server.exec_api(null, 'common-js/__mcp_exec', 'getScriptLogs', validMinutes)
          .then(function(response) {
            window.DataLoader.loadingLogs = false;
            if (!response || !response.success) {
              $content.html('<div class="log-empty">' + 
                (response && response.error ? response.error : 'Failed to load logs') + 
                '</div>');
              $status.text('Error');
              return;
            }
            ...
            if (!Array.isArray(response.logs)) {
              ...
              $content.html('<div class="log-empty">Invalid response format</div>');
              $status.text('Error');
              return;
            }

            const logs = response.logs;
            const timeRange = validMinutes === 0 ? 'all' : validMinutes + ' min';
    ...
    window.DeploymentManager = {
      ...
      loadDeploymentInfo: function() {
        // Prevent concurrent loads
        if (this.loading) {
          console.Logger.log('Deployment info already loading');
          return;
        }
        ...
        // Load URLs
        window.server.exec_api(null, 'common-js/__mcp_exec', 'getDeploymentUrls')
          .then((urls) => {
            if (!urls || typeof urls !== 'object') {
              console.error('Invalid deployment URLs response:', urls);
              this.loading = false;
              return;
            }
            ...
            if (!urls.dev || !urls.staging || !urls.prod) {
              console.error('Missing required URL properties:', urls);
              this.loading = false;
              return;
            }
        ...
        window.server.exec_api(null, 'common-js/__mcp_exec', 'getCurrentDeploymentType')
          .then((envType) => {
            const validEnvs = ['dev', 'staging', 'prod', 'unknown'];
            if (!validEnvs.includes(envType)) {
      ...
      promoteDeployment: function(toEnv) {
        // Validate parameters
        if (!toEnv || !['staging', 'prod'].includes(toEnv)) {
          window.UI.addOutput('❌ Invalid environment: ' + toEnv, 'error');
          return;
        }
        
        // Prevent concurrent promotions
        if (this.promoting) {
          window.UI.addOutput('⚠️ Promotion already in progress', 'info');
          return;
        }
        ...
        window.server.exec_api(null, 'common-js/__mcp_exec', 'promoteDeployment', toEnv, description)
          .then(function(result) {
            window.DeploymentManager.promoting = false;
            if (!result) {
              window.UI.addOutput('❌ Empty response from server', 'error');
              return;
            }
          ...
          .catch(function(error) {
            window.DeploymentManager.promoting = false;
            window.UI.addOutput(
              '❌ Promotion failed: ' + (error.message || 'Unknown error'),
              'error'
            );
          });
      }
    ...
    $(document).ready(function() {
      // Initialize Promise-based server API wrapper
      const server = createGasServer({ debug: false });
      window.server = server; // Make available globally for nested functions
      console.Logger.log('[Debug Console] Server API wrapper initialized');
      // Load script info
