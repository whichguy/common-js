function _main(
  ...
  /**
    ...
    class ClaudeConversation {
      constructor(apiKey, model = null) {
        ...
        if (model) {
          ...
          this.model = config.get('MODEL_NAME') || 'claude-haiku-4-5';
        }
        
        this.thinkingBudget = 2000; // Default thinking token budget
      ...
      sendMessage(params) {
        const {
          ...
          historicalAnchors = null  // Thread continuation context from previous threads
        } = params;
        ...
        if (!systemPrompt && depth === 0) {
          ...
          systemPrompt = this._buildSystemPrompt(knowledge, historicalAnchors);
        }
        ...
        Logger.log('[API_REQUEST] Model: ' + requestBody.model);
        Logger.log('[API_REQUEST] Messages count: ' + requestBody.messages.length);
        Logger.log('[API_REQUEST] Max tokens: ' + requestBody.max_tokens);
        Logger.log('[API_REQUEST] Tools count: ' + (requestBody.tools ? requestBody.tools.length : 0));
        Logger.log('[API_REQUEST] System prompt length: ' + (requestBody.system ? requestBody.system.length : 0) + ' chars');
        Logger.log('[API_REQUEST] Has thinking: ' + (requestBody.thinking ? 'yes' : 'no'));
        if (requestBody.messages.length > 0) {
          const lastMsg = requestBody.messages[requestBody.messages.length - 1];
          Logger.log('[API_REQUEST] Last message role: ' + lastMsg.role);
          Logger.log('[API_REQUEST] Last message content blocks: ' + (Array.isArray(lastMsg.content) ? lastMsg.content.length : 'not array'));
          if (Array.isArray(lastMsg.content) && lastMsg.content.length > 0) {
            Logger.log('[API_REQUEST] First content block type: ' + lastMsg.content[0].type);
            if (lastMsg.content[0].type === 'text') {
              Logger.log('[API_REQUEST] Text length: ' + (lastMsg.content[0].text ? lastMsg.content[0].text.length : 0) + ' chars');
              Logger.log('[API_REQUEST] Text preview: ' + (lastMsg.content[0].text ? lastMsg.content[0].text.substring(0, 200) : ''));
            }
          }
        }
    ...
    try {
      ...
      throw error;
        ...
        if (toolUses.length > 0 && this._toolRegistry) {
          ...
          if (onThinking) {
            onThinking(`[REGISTRY] Initialized: tools=${enabledToolNames}`, messageSequenceId);
          }
          
          const toolResults = toolUses.map(toolUse => {
            // Emit [TOOL_START] log
            const startTime = new Date().getTime();
            Logger.log(`[TOOL_START] tool=${toolUse.name} timestamp=${startTime}`);
            if (onThinking) {
              onThinking(`[TOOL_START] tool=${toolUse.name} timestamp=${startTime}`, messageSequenceId);
            }
            ...
            if (toolUse.name === 'exec' && toolUse.input.jsCode) {
              ...
              if (onThinking) {
                onThinking(`[EXEC_CODE] ${toolUse.input.jsCode}`, messageSequenceId);
              }
            }
            ...
            let success = true;
            try {
              toolResult = this._toolRegistry.executeToolCall(toolUse.name, toolUse.input, context);
            ...
            Logger.log(`[TOOL_END] tool=${toolUse.name} duration=${duration}ms success=${success} timestamp=${endTime}`);
            if (onThinking) {
              onThinking(`[TOOL_END] tool=${toolUse.name} duration=${duration}ms success=${success} timestamp=${endTime}`, messageSequenceId);
            }
            ...
            if (success && toolUse.name === 'exec' && toolResult.result !== undefined) {
              ...
              if (onThinking) {
                onThinking(`[SENSITIVE] ${resultJson}`, messageSequenceId);
              }
            }
    ...
    if (typeof checkControlMessages === 'function') {
      checkControlMessages();
        ...
        if (toolUses.length > 0) {
          ...
          const toolResults2 = toolUses.map(toolUse => {
            ...
            Logger.log(`[TOOL_START] tool=${toolUse.name} timestamp=${startTime}`);
            if (onThinking) {
              onThinking(`[TOOL_START] tool=${toolUse.name} timestamp=${startTime}`, sequenceId);
            }
            ...
            if (toolUse.name === 'exec' && toolUse.input.jsCode) {
              Logger.log(`[EXEC_CODE] ${toolUse.input.jsCode}`);
              if (onThinking) {
                onThinking(`[EXEC_CODE] ${toolUse.input.jsCode}`, sequenceId);
              }
            }
            ...
            Logger.log(`[TOOL_END] tool=${toolUse.name} duration=${duration}ms success=${success} timestamp=${endTime}`);
            if (onThinking) {
              onThinking(`[TOOL_END] tool=${toolUse.name} duration=${duration}ms success=${success} timestamp=${endTime}`, sequenceId);
            }
            ...
            if (success && toolUse.name === 'exec' && toolResult.result !== undefined) {
              ...
              Logger.log(`[SENSITIVE] ${resultJson}`);
              if (onThinking) {
                onThinking(`[SENSITIVE] ${resultJson}`, sequenceId);
              }
            }
       ...
       * @param {Array|null} historicalAnchors - Historical context from thread continuations
       * @returns {string} Complete system prompt
       */
      _buildSystemPrompt(knowledge = null, historicalAnchors = null) {
        ...
        // Fall back to default with historical anchors support
        Logger.log('[SystemPrompt] Using default prompt' + (historicalAnchors ? ` with ${historicalAnchors.length} historical anchors` : ''));
        return this._SystemPrompt.buildSystemPrompt(knowledge, historicalAnchors);
      }
