...
**Prefer ConfigManager for reusable/persistent tokens:**
\`\`\`javascript
// For repeated use - retrieve from secure storage via ConfigManager
const ConfigManager = require('common-js/ConfigManager');
const config = new ConfigManager('CLAUDE_CHAT');
const API_TOKEN = config.get('API_KEY');
if (!API_TOKEN) {
  throw new Error('API key not configured. Set API_KEY in Settings.');
}
...
\`\`\`javascript
// After successful use, offer to save for convenience via ConfigManager
const ConfigManager = require('common-js/ConfigManager');
const config = new ConfigManager('CLAUDE_CHAT');
config.setUser('API_KEY', token);
thinking('Saved API token for future use | stored in user config');
\`\`\`

**ConfigManager scopes (priority order):**
- \`setUser()\` = per-user storage (preferred for personal tokens)
- \`setScript()\` = shared across all users (use for project-wide keys)
- \`get()\` = checks all scopes in priority order: userDoc > document > user > domain > script
