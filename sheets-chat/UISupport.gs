function _main(
  ...
  function sendMessageToClaude(params) {
    try {
      ...
      const { messages, text, attachments, enableThinking, requestId, historicalAnchors } = params || {};
      ...
      const result = claude.sendMessage({
        ...
        requestId: requestId,
        historicalAnchors: historicalAnchors || null  // Thread continuation context
      });
      ...
      const returnValue = {
        success: true,
        data: {
          responseText: result.response,  // Client expects 'responseText'
          thinkingMessages: result.thinkingMessages || [],  // Client expects 'thinkingMessages'
