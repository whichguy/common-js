  // ⚠️ WARNING: This file is an HTML include - DO NOT add CommonJS wrapper!
     ...
     * If already processing, adds to queue instead of blocking
     */
    function sendMessage() {
      // Read input first to check if there's content to queue
      var $input = $('#messageInput');
      ...
      if (isProcessing) {
        ...
        addToQueue(message, currentAttachments);
        $input.val('');
        clearAllAttachments();
        updateSendButtonState();
        return;
      ...
      // Validate message or attachments present (already checked above for queue case)
      if (!hasMessage && !hasAttachments) {
        showToast('Please enter a message or attach files', 'error');
        return;
      }
      ...
      if (!hasAttachments) {
        ...
        currentAttachments.forEach(function(att) {
          if (ATTACHMENT_CONFIG.IMAGE_TYPES.indexOf(att.mediaType) !== -1) {
            messageContent.push({
              type: 'image',
              source: {
                type: 'base64',
          ...
          } else if (ATTACHMENT_CONFIG.DOCUMENT_TYPES.indexOf(att.mediaType) !== -1) {
            messageContent.push({
              type: 'document',
              source: {
                type: 'base64',
      ...
      // Create streaming thinking bubble AFTER user message (so it appears below)
      var streamContext = createStreamingThinkingBubble();
      ...
      // Save to command history (in-memory)
      if (hasMessage) {
        ...
        window.server.exec_api(
          ...
          console.warn('[History] Failed to persist command:', e);
        });
      }
        ...
        function(messages) {
          ...
          if (messages && messages.length > 0) {
            ...
            messages.forEach(function(msg) {
              var text = msg && (msg.text || msg.content);
              if (typeof text === 'string' && text.trim()) {
                // Simple dedup: skip if exact match to end of accumulated
                if (!streamContext.accumulatedThinking.endsWith(text)) {
                  // Add visual separator between sequential thinking blocks (Gemini-style)
                  if (streamContext.accumulatedThinking) {
                    // Use a subtle bullet separator that renders nicely in markdown
            ...
            // Queue throttled update to the thinking bubble
            queueThinkingUpdate(streamContext);
          }
        },
        {
          continuous: true,
      ...
      var controller = currentPollingController;
      
      currentCancellableCall
        .then(function(result) {
          ...
          if (!result || !result.success) {
            throw new Error(result && result.error ? result.error : 'Unknown error from server');
          }
          ...
          // Finalize the streaming thinking bubble (collapse, stop spinner)
          finalizeThinkingBubble(streamContext);
          ...
          // Auto-save conversation to Drive after each message exchange
        ...
        .catch(function(error) {
          ...
          // Finalize thinking bubble on error (remove if empty, otherwise collapse)
          finalizeThinkingBubble(streamContext);
          
          // Handle classified error from server or raw error
          var errorInfo = extractErrorInfo(error);
          
          // Show user-friendly error message in chat with optional retry
        ...
        .finally(function() {
          ...
          if (promptQueue.length > 0) {
            console.log('[Queue] Processing next queued item...');
